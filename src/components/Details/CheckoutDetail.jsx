...
function Row({ label, value, bold = false, valueColor }) {
  return (
    <div className="flex items-baseline justify-between" style={{ padding: '11px 0' }}>
      ...
      <span className="text-right" style={{ fontSize: '16px', fontWeight: bold ? 700 : 400, color: valueColor ?? '#60A5FA' }}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap" style={{ padding: '11px 0 0' }}>
      <div className="flex items-center gap-1.5">
        ...
        <span style={{ fontSize: '15px', fontWeight: 600, color: '#60A5FA' }}>{operatorName}</span>
      </div>
  ...
  return (
    <div style={{ padding: '4px 0' }}>
      <div style={{ padding: '12px 0 10px', borderBottom: '1px solid #F0F0F0' }}>
        <p style={{ fontSize: '20px', fontWeight: 700, color: '#60A5FA' }}>{record.itemName}</p>
