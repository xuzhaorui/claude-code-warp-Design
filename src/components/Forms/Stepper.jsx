...
export default function Stepper({ value, onChange, min = 1, max = 9999, label, hint, error, unit }) {
  ...
  return (
    <div>
      <div
        className={`relative flex items-center h-14 bg-bg-secondary rounded-2xl px-1.5 gap-0 border-2 transition-all duration-200 ${
      ...
      >
        ...
        <input
          ...
          placeholder="0"
          className={`flex-1 h-full text-center text-[22px] font-bold bg-transparent border-none outline-none text-text-primary placeholder:text-gray-300 placeholder:font-medium placeholder:text-base ${bounce ? 'animate-[stepperBounce_0.2s_ease]' : ''}`}
